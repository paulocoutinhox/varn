...
-- most ffi security ids (library loading, callbacks, abi, arbitrary memory, rce) are pure native or c++ concerns and are not reachable from this lua surface, so they are not driven here.
...
-- note: FFI-021 null pointer arg (strlen(nil)) is a native deref by design and is not exercised, because passing a null pointer to a dereferencing libc function crashes in libc, not in the binding.
