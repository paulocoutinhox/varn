...
Retry-with-backoff plus a tiny timer scheduler built on [async](../../modules/async). `retry.run` re-invokes a function until it succeeds or runs out of attempts, sleeping between tries with exponential backoff; `retry.after` and `retry.every` schedule one-shot and repeating work on background coroutines, each returning a cancellable handle.
