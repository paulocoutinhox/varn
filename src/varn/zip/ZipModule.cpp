#include "varn/zip/ZipModule.h"


#include "varn/async/Promise.h"
#include "varn/lua/LuaHelpers.h"
#include "varn/runtime/Runtime.h"

#include <lua.hpp>

#include <filesystem>
#include <fstream>
#include <stdexcept>
#include <string>
#include <string_view>
#include <utility>
#include <vector>

#if defined(VARN_HAVE_LIBZIP) && VARN_HAVE_LIBZIP
#include <zip.h>
#endif

namespace fs = std::filesystem;

namespace varn {

namespace {

#if defined(VARN_HAVE_LIBZIP) && VARN_HAVE_LIBZIP

Runtime& luaRuntime(lua_State* L) {
    return *static_cast<Runtime*>(varn::lua::LuaHelpers::getRuntime(L));
}

bool entryPathSafe(std::string_view entry) {
    if (entry.empty()) {
        return false;
    }
    if (entry.front() == '/' || entry.front() == '\\') {
        return false;
    }
    if (entry.find("..") != std::string_view::npos) {
        return false;
    }
    return true;
}

bool isSubpath(const fs::path& baseCanon, const fs::path& candidateCanon) {
    fs::path rel = fs::relative(candidateCanon, baseCanon);
    for (const auto& part : rel) {
        if (part == "..") {
            return false;
        }
    }
    return true;
}

void performExtract(const std::string& zipPath, const std::string& destDir) {
    int err = 0;
    zip_t* za = zip_open(zipPath.c_str(), ZIP_RDONLY, &err);
    if (!za) {
        throw std::runtime_error("zip_open failed (code " + std::to_string(err) + ")");
    }

    fs::create_directories(destDir);
    const fs::path destRoot = fs::canonical(fs::absolute(destDir));

    const zip_int64_t n = zip_get_num_entries(za, 0);
    for (zip_uint64_t i = 0; i < static_cast<zip_uint64_t>(n); ++i) {
        const char* name = zip_get_name(za, i, ZIP_FL_ENC_GUESS);
        if (name == nullptr) {
            continue;
        }
        const std::string nm(name);
        if (!entryPathSafe(nm)) {
            zip_discard(za);
            throw std::runtime_error("unsafe zip entry: " + nm);
        }

        const fs::path outFile = destRoot / nm;
        const fs::path parentDir = outFile.parent_path();
        fs::create_directories(parentDir);

        const fs::path canonParent = fs::weakly_canonical(parentDir);
        if (!isSubpath(destRoot, canonParent)) {
            zip_discard(za);
            throw std::runtime_error("zip slip blocked: " + nm);
        }

        if (!nm.empty() && nm.back() == '/') {
            continue;
        }

        zip_file_t* zf = zip_fopen_index(za, i, 0);
        if (!zf) {
            zip_discard(za);
            throw std::runtime_error("zip_fopen_index failed: " + nm);
        }

        std::ofstream out(outFile, std::ios::binary);
        if (!out) {
            zip_fclose(zf);
            zip_discard(za);
            throw std::runtime_error("cannot open output file: " + outFile.string());
        }

        char buf[8192];
        while (true) {
            const zip_int64_t r = zip_fread(zf, buf, sizeof(buf));
            if (r < 0) {
                zip_fclose(zf);
                zip_discard(za);
                throw std::runtime_error("zip_fread failed: " + nm);
            }
            if (r == 0) {
                break;
            }
            out.write(buf, static_cast<std::size_t>(r));
        }
        zip_fclose(zf);
    }

    if (zip_close(za) != 0) {
        throw std::runtime_error("zip_close failed after extract");
    }
}

void performCreate(const std::string& zipPath, const std::vector<std::pair<std::string, std::string>>& items) {
    int err = 0;
    zip_t* za = zip_open(zipPath.c_str(), ZIP_CREATE | ZIP_TRUNCATE, &err);
    if (!za) {
        throw std::runtime_error("zip_open(create) failed (code " + std::to_string(err) + ")");
    }

    for (const auto& [localPath, entryName] : items) {
        if (!entryPathSafe(entryName)) {
            zip_discard(za);
            throw std::runtime_error("unsafe zip entry name: " + entryName);
        }
        if (!fs::is_regular_file(localPath)) {
            zip_discard(za);
            throw std::runtime_error("not a regular file: " + localPath);
        }

        zip_source_t* s = zip_source_file(za, localPath.c_str(), 0, 0);
        if (!s) {
            zip_discard(za);
            throw std::runtime_error("zip_source_file failed: " + localPath);
        }
        const zip_int64_t idx = zip_file_add(za, entryName.c_str(), s, ZIP_FL_ENC_UTF_8);
        if (idx < 0) {
            zip_source_free(s);
            zip_discard(za);
            throw std::runtime_error("zip_file_add failed: " + entryName);
        }
    }

    if (zip_close(za) != 0) {
        throw std::runtime_error("zip_close failed after create");
    }
}

std::vector<std::string> performList(const std::string& zipPath) {
    int err = 0;
    zip_t* za = zip_open(zipPath.c_str(), ZIP_RDONLY, &err);
    if (!za) {
        throw std::runtime_error("zip_open failed (code " + std::to_string(err) + ")");
    }

    std::vector<std::string> names;
    const zip_int64_t n = zip_get_num_entries(za, 0);
    for (zip_uint64_t i = 0; i < static_cast<zip_uint64_t>(n); ++i) {
        const char* name = zip_get_name(za, i, ZIP_FL_ENC_GUESS);
        if (name == nullptr) {
            continue;
        }
        const std::string nm(name);
        if (!entryPathSafe(nm)) {
            zip_discard(za);
            throw std::runtime_error("unsafe zip entry: " + nm);
        }
        names.push_back(nm);
    }

    if (zip_close(za) != 0) {
        throw std::runtime_error("zip_close failed after list");
    }
    return names;
}

#endif

} // namespace (anonymous)

int ZipModule::luaExtract(lua_State* L) {
#if defined(VARN_HAVE_LIBZIP) && VARN_HAVE_LIBZIP
    const std::string zipPath = varn::lua::LuaHelpers::checkString(L, 1);
    const std::string destDir = varn::lua::LuaHelpers::checkString(L, 2);
    auto& rt = luaRuntime(L);
    auto promise = std::make_shared<Promise>(rt);
    rt.taskPool().post([promise, zipPath, destDir] {
        try {
            performExtract(zipPath, destDir);
            promise->resolve("ok");
        } catch (const std::exception& ex) {
            promise->reject(ex.what());
        }
    });
    Promise::push(L, promise);
    return 1;
#else
    return luaL_error(L, "zip.extract is unavailable (Emscripten or VARN_ENABLE_ZIP=OFF / libzip not linked)");
#endif
}

int ZipModule::luaCreate(lua_State* L) {
#if defined(VARN_HAVE_LIBZIP) && VARN_HAVE_LIBZIP
    const std::string zipPath = varn::lua::LuaHelpers::checkString(L, 1);
    luaL_checktype(L, 2, LUA_TTABLE);
    const int len = static_cast<int>(lua_rawlen(L, 2));
    if (len <= 0) {
        return luaL_error(L, "zip.create: entries array must be non-empty");
    }

    std::vector<std::pair<std::string, std::string>> items;
    items.reserve(static_cast<std::size_t>(len));

    for (int i = 1; i <= len; ++i) {
        lua_rawgeti(L, 2, i);
        if (!lua_istable(L, -1)) {
            lua_pop(L, 1);
            return luaL_error(L, "zip.create: each entry must be a table");
        }
        lua_getfield(L, -1, "file");
        const std::string file = varn::lua::LuaHelpers::checkString(L, -1);
        lua_pop(L, 1);
        lua_getfield(L, -1, "entry");
        const std::string entry = varn::lua::LuaHelpers::checkString(L, -1);
        lua_pop(L, 1);
        lua_pop(L, 1);
        items.emplace_back(file, entry);
    }

    auto& rt = luaRuntime(L);
    auto promise = std::make_shared<Promise>(rt);
    rt.taskPool().post([promise, zipPath, items = std::move(items)] {
        try {
            performCreate(zipPath, items);
            promise->resolve("ok");
        } catch (const std::exception& ex) {
            promise->reject(ex.what());
        }
    });
    Promise::push(L, promise);
    return 1;
#else
    return luaL_error(L, "zip.create is unavailable (Emscripten or VARN_ENABLE_ZIP=OFF / libzip not linked)");
#endif
}

int ZipModule::luaList(lua_State* L) {
#if defined(VARN_HAVE_LIBZIP) && VARN_HAVE_LIBZIP
    const std::string zipPath = varn::lua::LuaHelpers::checkString(L, 1);
    auto& rt = luaRuntime(L);
    auto promise = std::make_shared<Promise>(rt);
    rt.taskPool().post([promise, zipPath] {
        try {
            std::vector<std::string> names = performList(zipPath);
            promise->resolveCustom([names = std::move(names)](lua_State* lua) {
                lua_createtable(lua, static_cast<int>(names.size()), 0);
                int idx = 1;
                for (const auto& s : names) {
                    lua_pushlstring(lua, s.data(), s.size());
                    lua_rawseti(lua, -2, idx);
                    ++idx;
                }
            });
        } catch (const std::exception& ex) {
            promise->reject(ex.what());
        }
    });
    Promise::push(L, promise);
    return 1;
#else
    return luaL_error(L, "zip.list is unavailable (Emscripten or VARN_ENABLE_ZIP=OFF / libzip not linked)");
#endif
}

int ZipModule::luaOpen(lua_State* L) {
    lua_newtable(L);
    lua_pushcfunction(L, &ZipModule::luaExtract);
    lua_setfield(L, -2, "extract");
    lua_pushcfunction(L, &ZipModule::luaCreate);
    lua_setfield(L, -2, "create");
    lua_pushcfunction(L, &ZipModule::luaList);
    lua_setfield(L, -2, "list");
    return 1;
}

void ZipModule::install(lua_State* L) {
    luaL_requiref(L, "zip", &ZipModule::luaOpen, 1);
    lua_pop(L, 1);
}

} // namespace varn
