...
- Module Lua tests under `modules/*/lua/tests/*.lua` are auto-discovered, but a **component** test only runs in CI once it is added to `_COMPONENT_TESTS` (no backend needed) or `_DB_TESTS` (needs one) in `tools/core/tests.py`. Those lists are explicit, so a name that is not on disk **fails the run** rather than being skipped — a silent skip hides a file that never reached the repository.
...
- Docs are two-tier, and **both tiers exist for every module and component**. Tier one is the `README.md` next to the code: a short intro, a capabilities table, and a closing `## Reference, examples, and tests` section linking the reference page, `examples/`, and `tests/`. Tier two is the full reference under `docs/lua-api/<module>.md` or `docs/components/<component>.md` — the complete API plus every runnable example inlined verbatim from `examples/`. `docs/lua-api.md` and `docs/components.md` index them, and every row of those tables points at a `docs/` page, never at a `README.md`. Adding a module or component means adding its reference page and its index row in the same change.
