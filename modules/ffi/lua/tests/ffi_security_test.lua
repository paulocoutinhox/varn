-- ffi security covering the lua-reachable subset of argument marshalling (argument type validation the binding checks before the call) while pure native or c++ ids (library loading, callbacks, abi, arbitrary memory, rce) are not reachable from this lua surface, exercising FFI-023 type confusion string vs pointer and FFI-028 float/int coercion to a wrong c type but not FFI-021 null pointer arg (strlen(nil)) which is a native deref crashing in libc rather than the binding, staying inside the always-present process namespace ffi.C with standard libc symbols.
